...
namespace SbnSharp
{
    ...
    internal class OldSbnBin
    {
        ...
        internal OldSbnBin(BinaryReader reader)
...
#if VERBOSE
            Console.WriteLine(" {0, 4}", numFeatures);
#endif
        ...
        /// </summary>
        /// <param name="writer">The writer</param>
        /// <param name="sbxWriter">The writer to update the sbx-index file</param>
        internal void Write(BinaryWriter writer, BinaryWriter sbxWriter)
        {
            var block = 0;
            while (block * 100 < NumFeatures)
            {
                sbxWriter.WriteBE((int)writer.BaseStream.Position / 2);
                sbxWriter.WriteBE(Math.Min(100, NumFeatures - block * 100) *4);
